class Trainee < ApplicationRecord
  ...
  def self.trainees_by_month_by_type(month, type, org_unit)
    joins(:training).where('trainings.end_time >= ? and trainings.end_time <= ? and trainings.training_type = ? and person_id in (?)',
                           month.beginning_of_month,  month.end_of_month, type, org_unit.trainees.pluck(:id) ).count
  end
